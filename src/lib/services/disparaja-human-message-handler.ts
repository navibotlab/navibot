 ...
 * ao thread do OpenAI Assistant no canal DisparaJá, mantendo o contexto da conversa atualizado.
 */
export class DisparaJaHumanMessageHandler {
  private openai: OpenAI;
  private logger: SimpleLogger;
  
  /**
   * Cria uma nova instância do manipulador de mensagens humanas para o canal DisparaJá
  ...
  private async verifyAndHandleActiveRuns(threadId: string, logContext: any): Promise<boolean> {
    try {
      ...
      this.logger.info(`🔍 [DisparaJa] Verificando runs ativos no thread ${threadId}...`, logContext);
      ...
      if (!runsList || !runsList.data || runsList.data.length === 0) {
        this.logger.info(`✅ [DisparaJa] Nenhum run ativo encontrado no thread`, logContext);
        return true;
      }
      ...
      this.logger.info(`📌 [DisparaJa] Run mais recente: ${latestRun.id} com status: ${runStatus}`, logContext);
      ...
      if (finishedStates.includes(runStatus)) {
        this.logger.info(`✅ [DisparaJa] Run ${latestRun.id} já finalizado com status: ${runStatus}`, logContext);
        return true;
      }
      
      // Se o run está em andamento, tentar cancelá-lo
      if (['in_progress', 'queued', 'requires_action'].includes(runStatus)) {
        this.logger.info(`⚠️ [DisparaJa] Run ${latestRun.id} está ativo com status: ${runStatus}, tentando cancelar...`, logContext);
        
        try {
          await this.openai.beta.threads.runs.cancel(threadId, latestRun.id);
          this.logger.info(`✅ [DisparaJa] Run ${latestRun.id} cancelado com sucesso`, logContext);
          ...
          this.logger.error(`❌ [DisparaJa] Não foi possível cancelar run ${latestRun.id}: ${cancelError.message}`, logContext);
          ...
          try {
            ...
            if (finishedStates.includes(currentRun.status)) {
              this.logger.info(`✅ [DisparaJa] Run ${latestRun.id} finalizou durante a verificação: ${currentRun.status}`, logContext);
              return true;
            }
            
            this.logger.error(`❌ [DisparaJa] Run ${latestRun.id} ainda ativo e não pôde ser cancelado: ${currentRun.status}`, logContext);
            return false;
          } catch (retrieveError) {
            this.logger.error(`❌ [DisparaJa] Erro ao verificar status atual do run: ${retrieveError}`, logContext);
            return false;
          }
        }
      }
      ...
      this.logger.error(`❌ [DisparaJa] Erro ao verificar runs ativos: ${error.message}`, logContext);
  ...
  async addMessageToThread(
    ...
    try {
      ...
      if (!threadId) {
        this.logger.error('❌ [DisparaJa] ThreadId não fornecido ou inválido', logContext);
        return { success: false, message: 'ThreadId inválido' };
      }
      
      // Log do início do processo
      this.logger.info('🔵 [DisparaJa] INICIANDO ADIÇÃO DE MENSAGEM HUMANA AO THREAD', logContext);
      this.logger.info(`[DisparaJa] Thread ID: ${threadId}`, logContext);
      this.logger.info(`[DisparaJa] Conteúdo da mensagem: "${content}"`, logContext);
      
      // Verificar se o thread existe antes de tentar adicionar a mensagem
      try {
        this.logger.info(`[DisparaJa] Verificando existência do thread ${threadId}...`, logContext);
        await this.openai.beta.threads.retrieve(threadId);
        this.logger.info(`✅ [DisparaJa] Thread ${threadId} encontrado`, logContext);
      } catch (threadError: any) {
        this.logger.error(`❌ [DisparaJa] Thread ${threadId} não encontrado ou inacessível:`, threadError);
        return { 
          success: false, 
      ...
      this.logger.info(`[DisparaJa] Adicionando mensagem humana ao thread ${threadId}...`, logContext);
      await this.openai.beta.threads.messages.create(threadId, {
        role: 'assistant',
        content: `[MENSAGEM ENVIADA PELO OPERADOR HUMANO VIA DISPARAJA]: ${content}`
      });
      
      this.logger.info('✅ [DisparaJa] Mensagem humana adicionada ao thread com sucesso', logContext);
      
      // Se um assistantId foi fornecido, criar um run para processar a mensagem
      if (assistantId) {
        this.logger.info(`[DisparaJa] Criando run com assistantId: ${assistantId}`, logContext);
        try {
          // Verificar se o assistente existe antes de criar o run
          try {
            this.logger.info(`[DisparaJa] Verificando existência do assistente ${assistantId}...`, logContext);
            await this.openai.beta.assistants.retrieve(assistantId);
            this.logger.info(`✅ [DisparaJa] Assistente ${assistantId} encontrado`, logContext);
          } catch (assistantError: any) {
            this.logger.error(`❌ [DisparaJa] Assistente ${assistantId} não encontrado ou inacessível:`, assistantError);
            return { 
          ...
          this.logger.info(`✅ [DisparaJa] Run ${run.id} criado com sucesso`, logContext);
          return { 
            success: true, 
          ...
          this.logger.error('⚠️ [DisparaJa] Erro ao criar run após adicionar mensagem humana:', {
            ...logContext,
            errorMessage,
            errorStatus,
            errorCode,
      ...
      this.logger.info('🔵 [DisparaJa] FIM DA ADIÇÃO DE MENSAGEM HUMANA AO THREAD', logContext);
      return { success: true, message: 'Mensagem adicionada ao thread com sucesso' };
    } catch (error: any) {
      this.logger.error('❌ [DisparaJa] Erro ao adicionar mensagem humana ao thread:', error);
      return { 
        success: false, 
